...
const asyncFunc = async () => {
  ...
  for (let i = 0; i < projectIDs.length; i++) {
    const result = await axios.get(`${process.env.MY_SPACE}/api/v2/projects/${projectIDs[i]}/users?apiKey=${process.env.API_KEY}`)
      .catch(() => console.log('エラー'))
    const userIds = result.data.map(e => [e.id, e.name, projectIDs[i]])
    const userCSV = result.data.map(e => [e.id, e.name, projectIDs[i]].join(','))
